...
# ex : http://www.xiuren.org/bololi/044/00##.jpg
num_pic = input("enter the number of images :")
if os.path.isdir('./XIUREN_temp'):
	shutil.rmtree('./XIUREN_temp')
	os.mkdir('./XIUREN_temp')
else: 
	os.mkdir('./XIUREN_temp')
...
def dojob(*args):
	...
	while True:
		...
		if pic_count > int(num_pic):
			break
		file_name = str(pic_count)
		f = open('./XIUREN_temp/'+file_name+'.jpg','wb')
		f.write(r.content)
		f.close
		pic_count += 3
		lock.acquire()
